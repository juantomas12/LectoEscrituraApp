...
class _SoundMatchScreenState extends ConsumerState<SoundMatchScreen> {
  ...
  String _feedback = 'ESCUCHA EL SONIDO Y TOCA LA IMAGEN CORRECTA';
  ...
  Future<void> _prepare({List<Item>? reinforcementTargets}) async {
    ...
    _pool = dataset
        ...
        .where((item) {
          ...
          final hasAudio = (item.audioAsset ?? '').trim().isNotEmpty;
          final hasTts = (item.ttsText ?? '').trim().isNotEmpty;
          return word.isNotEmpty && (hasAudio || hasTts);
        })
        .toList();
    ...
    setState(() {
      ...
      _feedback = 'ESCUCHA EL SONIDO Y TOCA LA IMAGEN CORRECTA';
  ...
  void _prepareRound() {
    ...
    final distractors =
        _pool
            .where((item) => item.id != target.id && item.word != target.word)
            .toList()
          ..shuffle(_random);
    ...
    _scheduleAutoPlayForRound(target.id);
  }

  void _scheduleAutoPlayForRound(String itemId) {
    Future<void>.delayed(const Duration(milliseconds: 250), () async {
      if (!mounted || _target?.id != itemId) {
        return;
      }
  ...
  Future<void> _playCurrentSound() async {
    ...
    if (target == null || _isPlayingSound) {
      return;
    }

    final audioAsset = (target.audioAsset ?? '').trim();
    final ttsText = (target.ttsText ?? target.word ?? '').trim();
  ...
  Future<void> _answer(Item selected) async {
    ...
    await ref
        ...
        .registerAttempt(
          itemId: _target!.id,
          correct: isCorrect,
          activityType: ActivityType.sonidos,
        );
  ...
  Widget build(BuildContext context) {
    ...
    return GameScaffold(
      ...
      instructionText: 'ESCUCHA EL SONIDO Y TOCA LA IMAGEN CORRECTA',
      progressCurrent: solvedCount,
      progressTotal: _roundTargets.length,
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _target == null
          ? const Center(child: UpperText('NO HAY SONIDOS DISPONIBLES'))
          : Center(
